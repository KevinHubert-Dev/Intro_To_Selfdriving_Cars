...
vector< vector <float> > initialize_beliefs(vector< vector <char> > grid) {
	...
	/* The code expects that each row as the same amount of cells */
	int gridItems = grid.size() * grid[0].size();
	float uniformDistributionValue = (float)1 / gridItems; /* type double would be more precise but the result is a grid of floats */
  
	for (int i = 0; i < newGrid.size(); i++) {
		for (int j = 0; j < newGrid[i].size(); j++) {
			newGrid[i][j] = uniformDistributionValue;
		}
	}
	return newGrid;
}
  ...
  float blurring) 
{
  ...
  for (int i = 0; i < beliefs.size(); i++) {
  	for (int j = 0; j < beliefs[0].size(); j++) {
    	newGrid[(((i + dy) % beliefs.size()) + beliefs.size()) % beliefs.size()][(((j + dx) % beliefs[i].size()) + beliefs[i].size()) % beliefs[i].size()] = beliefs[i][j];
    }
  }
