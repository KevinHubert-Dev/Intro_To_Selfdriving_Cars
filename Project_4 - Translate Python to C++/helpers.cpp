...
vector< vector<float> > normalize(vector< vector <float> > grid) {
	...
	/* Count sum of all values contained in the grid */
	float sum = 0;
	...
	/* Fill new grid with normalized values */
	for(int i = 0; i < grid.size(); i++) {
		for (int j = 0; j < grid[i].size(); j++) {
...
vector < vector <float> > blur(vector < vector < float> > grid, float blurring) {
	...
	//	1/12   1/06   1/12
	if (blurring > 1) {
        throw std::invalid_argument( "blurring greater than 1 is not allowed" );
    }
    
  
    vector < vector < float > > blurrGrid {
      { (float)(blurring / 12), (float)(blurring / 6), (float)(blurring / 12) },
      { (float)(blurring / 6),  1 - blurring , (float)(blurring / 6)  },
      { (float)(blurring / 12), (float)(blurring/6), (float)(blurring / 12) }
    };
	...
	for (int i = 0; i < grid.size(); i++) {
		/* Iterate through cols in grid */
		for (int j = 0; j < grid[i].size(); j++) {
			/* Iterate through blurrGrid-Rows */
			for (int ib = 0; ib < blurrGrid.size(); ib++) {
				/* Iterate through blurrGrid-Rows */
				for (int jb = 0; jb < blurrGrid[ib].size(); jb++) {
                  int currGridX = (((i + ib - 1) % grid.size()) + grid.size() % grid.size());
                  int currGridY = (((j + jb - 1) % grid[0].size()) + grid[0].size() % grid[0].size());
                  float currCellVal = grid[i][j];
				  newGrid[currGridX][currGridY] += (blurrGrid[ib][jb] * currCellVal);
				}
			}
		}
	}
